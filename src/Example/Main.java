...
public class Main {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < 100; i++) {
            if(i < 50)
                computers.add(new Laptop(100 * i + 10, ComputerModel.Acer));
            else
                computers.add(new Pc(50 * i + 10, ComputerModel.Hp, 24));
        }
        ...
        for(int i = 0; i < computers.size(); i++) {
            if(computers.get(i) instanceof Laptop)
                laptopSum += computers.get(i).getPrice();
            if(computers.get(i) instanceof Pc)
                pcSum += computers.get(i).getPrice();
        }
